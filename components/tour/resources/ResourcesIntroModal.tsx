...
export default function ResourcesIntroModal() {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Resources walkthrough"
      style={{
        ...
        padding: 20,
        animation: "rs-modal-bg 0.2s ease-out",
      }}
    >
      <style>{`
        @keyframes rs-modal-bg   { from { opacity: 0; } to { opacity: 1; } }
        @keyframes rs-modal-card { from { opacity: 0; transform: scale(0.96) translateY(8px); } to { opacity: 1; transform: scale(1) translateY(0); } }
      `}</style>
      ...
      <div
        style={{
          ...
          animation: "rs-modal-card 0.24s ease-out",
        }}
      >
        {/* Header */}
        <div style={{
          ...
          <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--color-grey)" }}>
            Resources · {stepIdx + 1} of {SLIDES.length}
          </span>
          <button
            onClick={() => close(false)}
