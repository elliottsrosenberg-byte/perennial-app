  ...
  return (
    <div
      ...
      style={{
        ...
        padding: 20,
        animation: "tk-modal-bg 0.2s ease-out",
      }}
    >
      <style>{`
        @keyframes tk-modal-bg   { from { opacity: 0; } to { opacity: 1; } }
        @keyframes tk-modal-card { from { opacity: 0; transform: scale(0.96) translateY(8px); } to { opacity: 1; transform: scale(1) translateY(0); } }
      `}</style>
      ...
      <div
        style={{
          ...
          animation: "tk-modal-card 0.24s ease-out",
        }}
      >
        {/* Header */}
        <div style={{
          ...
          <span style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.07em", color: "var(--color-grey)" }}>
            Tasks · {stepIdx + 1} of {SLIDES.length}
          </span>
          <button
            onClick={() => close(false)}
