...
// Static display metadata per category — labels, colours, icon paths never change
const CAT_META: Record<Exclude<CatId,"links">, { label: string; sub: string; iconBg: string; iconColor: string; iconSvg: string }> = {
  ...
  press:      { label:"Press",      sub:"Media kit, pitch decks, and press coverage",   iconBg:C.blueL,   iconColor:C.blue,   iconSvg:`<path d="M2 2h12v9H2z"/><path d="M5 6h6M5 9h4"/>` },
  design:     { label:"Design",     sub:"Templates, product photos, and working files", iconBg:C.accentL, iconColor:C.accent, iconSvg:`<path d="M2 12L10 4l4 4-8 8-4 0 0-4z"/>` },
};
...
const MODALS: Record<string, ModalConfig> = {
  missionvision: {
    ...
    iconBg:C.accentL, iconColor:C.accent,
...
function HealthPip({ filled, total }: { filled: number; total: number }) {
  ...
  const color = pct >= 0.8 ? C.accent : pct >= 0.35 ? C.amber : "rgba(31,33,26,0.22)";
...
function ActionBtn({ action, onOpenModal }: { action: CardAction; onOpenModal: (k: string) => void }) {
  ...
  if (action.variant === "finder") {
    return (
      <button onClick={handleClick} title="Available in desktop app — opens file in Finder"
        style={{ display:"flex", alignItems:"center", gap:3, padding:"3px 8px", fontSize:10, borderRadius:5, cursor:"pointer", border:"0.5px solid var(--color-border)", background:"transparent", color:"var(--color-grey)", fontFamily:"inherit", opacity:0.75 }}>
  ...
  if (action.variant === "primary") {
    return (
      <button onClick={handleClick}
        style={{ padding:"3px 9px", fontSize:10, borderRadius:5, cursor:"pointer", border:"none", background:C.accent, color:"white", fontFamily:"inherit", fontWeight:500, whiteSpace:"nowrap" }}>
        {action.label}
      </button>
    );
  }
...
function CardPreview({ type, data }: { type: string; data?: Record<string, any> }) {
  ...
  if (type === "files") return (
    <div style={{ ...base, background:"var(--color-cream)", flexDirection:"column", alignItems:"flex-start", gap:4, padding:"10px 14px" }}>
      ...
      {badge(`${d.count} files`, C.accent, C.accentL)}
    </div>
  );
  ...
  if (type === "logos") return (
    <div style={{ ...base, background:"var(--color-cream)", gap:10 }}>
      ...
      {badge("3 files", C.accent, C.accentL)}
    </div>
  );
  ...
  if (type === "linked") return (
    <div style={{ ...base, background:C.blueL, flexDirection:"column", gap:4 }}>
      <span style={{ color:C.blue }}><IcLink /></span>
      <span style={{ fontSize:10, color:C.blue, fontWeight:500 }}>{d.service}</span>
    ...
    >
      <div style={{ background:"var(--color-off-white)", borderRadius:12, boxShadow:"0 8px 40px rgba(0,0,0,0.16), 0 0 0 0.5px rgba(0,0,0,0.07)", width:"100%", maxWidth:540, maxHeight:"82vh", display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        <div style={{ display:"flex", alignItems:"flex-start", gap:12, padding:"18px 20px 14px", borderBottom:"0.5px solid var(--color-border)" }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ fontSize:16, fontWeight:650 }}>{cfg.title}</div>
        ...
        <div style={{ flex:1, overflowY:"auto", padding:20 }}>
          {/* Why card */}
          <div style={{ background:C.accentL, border:"0.5px solid rgba(61,107,79,0.18)", borderRadius:8, padding:"12px 14px", fontSize:11, color:"var(--color-grey)", lineHeight:1.55, marginBottom:18 }}>
            {cfg.why}
          </div>

          {/* Ash CTA */}
          <div style={{ display:"flex", alignItems:"center", gap:14, padding:"16px 18px", background:"var(--color-charcoal)", borderRadius:12, cursor:"pointer", marginBottom:10 }}>
            <div style={{ width:38, height:38, borderRadius:"50%", background:"rgba(255,255,255,0.15)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="1.5"><path d="M12 22c0 0-6-4-6-10a6 6 0 0 1 12 0c0 6-6 10-6 10z"/><path d="M12 12c-2-1.5-3-3-2-5"/><path d="M12 12c2-1.5 3-3 2-5"/></svg>
            </div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:13, fontWeight:650, color:"white" }}>Draft this with Ash</div>
              <div style={{ fontSize:11, color:"rgba(255,255,255,0.55)", marginTop:2 }}>Answer a few quick questions and Ash writes a first draft — takes about 2 minutes</div>
            </div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, margin:"12px 0" }}>
            ...
            <span style={{ fontSize:10, color:"var(--color-grey)" }}>or</span>
            <div style={{ flex:1, height:"0.5px", background:"var(--color-border)" }} />
          </div>

          <button onClick={() => setShowManual(v => !v)} style={{ display:"block", width:"100%", textAlign:"center", fontSize:11, color:"var(--color-grey)", cursor:"pointer", padding:6, borderRadius:6, background:"none", border:"none", fontFamily:"inherit" }}>
            {showManual ? "Hide manual prompts ↑" : "Fill in manually →"}
          </button>

          {/* Manual prompts */}
          {showManual && (
            <div style={{ marginTop:16, paddingTop:16, borderTop:"0.5px solid var(--color-border)" }}>
              <div style={{ display:"flex", alignItems:"center", gap:8, fontSize:10, color:"var(--color-grey)", marginBottom:14 }}>
                <span>0 of {cfg.prompts.length} complete</span>
                <div style={{ flex:1, height:3, background:"var(--color-cream)", borderRadius:2, overflow:"hidden" }}>
                  <div style={{ height:"100%", background:C.amber, borderRadius:2, width:"0%" }} />
                </div>
              </div>
              {cfg.prompts.map((p, i) => (
                <div key={i} style={{ marginBottom:12 }}>
                  <div style={{ fontSize:11, fontWeight:600, color:"var(--color-grey)", marginBottom:4 }}>{p.label}</div>
              ...
              <div style={{ border:"0.5px dashed var(--color-border)", borderRadius:8, padding:16, textAlign:"center", cursor:"pointer" }}>
                <div style={{ fontSize:11, color:"var(--color-grey)" }}>Drop a file or click to upload</div>
        ...
        <div style={{ padding:"12px 20px", borderTop:"0.5px solid var(--color-border)", display:"flex", gap:7 }}>
          ...
          <button style={{ flex:1.5, padding:"7px 0", fontSize:11, border:"none", borderRadius:6, background:C.accent, color:"white", fontWeight:500, cursor:"pointer", fontFamily:"inherit" }}>Save draft</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 0", marginBottom:12 }}>
        <span style={{ fontSize:12, color:"var(--color-grey)" }}>{links.length} saved links</span>
        <button onClick={onAddLink} style={{ display:"flex", alignItems:"center", gap:4, padding:"4px 10px", fontSize:11, border:"0.5px solid var(--color-border)", borderRadius:6, background:"transparent", color:"var(--color-grey)", cursor:"pointer", fontFamily:"inherit" }}>
          <IcPlus /> Add link
        </button>
      </div>
      <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
        {links.map(link => (
          <div key={link.id} className="flex items-center gap-3" style={{ padding:"11px 15px", background:"var(--color-off-white)", borderRadius:10, boxShadow:"0 1px 4px rgba(0,0,0,0.07), 0 0 0 0.5px rgba(0,0,0,0.07)", cursor:"pointer" }}>
            <div style={{ width:28, height:28, borderRadius:7, background:C.blueL, display:"flex", alignItems:"center", justifyContent:"center", color:C.blue, flexShrink:0 }}><IcLink /></div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontSize:12, fontWeight:600 }}>{link.name}</div>
...
// ─── Category nav ─────────────────────────────────────────────────────────────
function CategoryNav({ active, resources, links, onSelect }: { active: CatId; resources: Resource[]; links: ResourceLink[]; onSelect: (id: CatId) => void }) {
  const [search, setSearch] = useState("");

  return (
    <div style={{ width:204, flexShrink:0, background:"var(--color-off-white)", borderRight:"0.5px solid rgba(31,33,26,0.13)", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      {/* Header */}
      <div style={{ padding:"14px 16px 10px", borderBottom:"0.5px solid var(--color-border)", flexShrink:0 }}>
        <div style={{ fontSize:14, fontWeight:650 }}>Resources</div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"8px 0" }}>
        ...
        {CAT_IDS.map(id => {
          ...
          return (
            <div
              ...
              style={{ padding:"8px 14px", cursor:"pointer", borderLeft:`2.5px solid ${active===id?"var(--color-charcoal)":"transparent"}`, background:active===id?"var(--color-cream)":undefined }}
            >
        ...
        <div
          ...
          className="flex items-center gap-2"
          style={{ padding:"8px 14px", cursor:"pointer", borderLeft:`2.5px solid ${active==="links"?"var(--color-charcoal)":"transparent"}`, background:active==="links"?"var(--color-cream)":undefined }}
        >
        ...
        <div style={{ padding:"10px 14px" }}>
          <button style={{ width:"100%", padding:"7px 10px", fontSize:11, color:"var(--color-grey)", background:"transparent", border:"0.5px dashed var(--color-border)", borderRadius:6, cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:6, fontFamily:"inherit" }}>
            <IcPlus /> Add category
          </button>
        </div>
      </div>
...
export default function ResourcesClient({ initialResources, initialLinks }: { initialResources: Resource[]; initialLinks: ResourceLink[] }) {
  ...
  const catCards = !isLinks ? initialResources.filter(r => r.category === cat).map(resourceToCard) : [];
  ...
  return (
    <div className="flex h-full overflow-hidden">
      <CategoryNav active={cat} resources={initialResources} links={initialLinks} onSelect={setCat} />

      <div className="flex flex-col flex-1 overflow-hidden">
        {/* Topbar */}
        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"11px 20px", borderBottom:"0.5px solid var(--color-border)", background:"var(--color-off-white)", flexShrink:0 }}>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:14, fontWeight:650 }}>{sectionMeta[cat].title}</div>
          ...
          {!isLinks && (
            <>
              ...
              <button style={{ padding:"5px 12px", fontSize:11, borderRadius:6, cursor:"pointer", border:"0.5px solid var(--color-border)", background:"transparent", color:"var(--color-grey)", fontFamily:"inherit" }}>Add resource</button>
            </>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto" style={{ padding:20 }}>
          {isLinks && <LinksView links={initialLinks} onAddLink={() => {}} />}
          ...
          {!isLinks && catMeta && (
            <>
              ...
              <div className="flex items-center gap-3" style={{ padding:"10px 14px", background:"var(--color-off-white)", borderRadius:8, boxShadow:"0 1px 4px rgba(0,0,0,0.07), 0 0 0 0.5px rgba(0,0,0,0.07)", marginBottom:18 }}>
                <span style={{ fontSize:11, color:"var(--color-grey)", flex:1 }}>{catMeta.label} profile complete</span>
                <div style={{ flex:2, height:4, background:"var(--color-cream)", borderRadius:3, overflow:"hidden" }}>
                  <div style={{ height:"100%", borderRadius:3, background:C.accent, width:`${healthTotal > 0 ? (healthFilled/healthTotal)*100 : 0}%` }} />
                </div>
                <span style={{ fontSize:11, fontWeight:600, color:"var(--color-grey)" }}>{healthFilled} / {healthTotal}</span>
                <button onClick={() => { const empty = catCards.find(c => c.status === "empty" && c.modalKey); if (empty?.modalKey) setModal(empty.modalKey); }} style={{ fontSize:11, color:C.blue, background:"none", border:"none", cursor:"pointer", fontFamily:"inherit", padding:0 }}>Complete profile →</button>
              </div>
              ...
              {view === "list" && (
                <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                  {catCards.map(card => (
                    <div key={card.id} onClick={() => card.modalKey && setModal(card.modalKey)} className="flex items-center gap-4" style={{ padding:"11px 15px", background:"var(--color-off-white)", borderRadius:10, boxShadow:"0 1px 4px rgba(0,0,0,0.07), 0 0 0 0.5px rgba(0,0,0,0.07)", cursor:card.modalKey?"pointer":"default" }}>
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        <span style={{ fontSize:9, fontWeight:600, textTransform:"uppercase", letterSpacing:"0.04em", padding:"2px 7px", borderRadius:10,
                          background: card.status==="complete"?C.accentL : card.status==="partial"?C.amberL : card.status==="alias"?C.blueL : "var(--color-cream)",
                          color: card.status==="complete"?C.accent : card.status==="partial"?C.amber : card.status==="alias"?C.blue : "var(--color-grey)",
                        }}>{card.status}</span>
