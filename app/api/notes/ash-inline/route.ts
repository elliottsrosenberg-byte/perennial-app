...
interface ViewSpec {
   ...
   *  that detail panel (route to /network?contactId=…&tab=…&taskId=…)
...
function panelHref(surface: Surface, tab: string, highlight?: { key: string; value: string }): string {
  ...
  let base = "";
  if (surface.type === "canvas-contact" && surface.contact_id) {
    base = `/network?contactId=${surface.contact_id}&tab=${tab}`;
    ...
    if (surface.contact_id)      base = `/network?contactId=${surface.contact_id}&tab=${tab}`;
...
const VIEW_FOR_TOOL: Record<string, ViewSpec> = {
  ...
  create_contact: {
    label: "View contact",
    href: (id) => id ? `/network?contactId=${id}` : "/network",
  },
  log_time: {
    label: "View time",
  ...
  log_contact_activity: {
    ...
    href: (_id, input, surface) => {
      ...
      return cid ? `/network?contactId=${cid}&tab=activity` : "/network";
    },
  },
};
